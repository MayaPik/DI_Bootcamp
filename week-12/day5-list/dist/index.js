...
function createList(arr) {
    ...
    arr.forEach((one) => {
        ...
        function editTheTask(event) {
            ...
            document.getElementById('start').value = one.start.toISOString().slice('T', 10);
            document.getElementById('end').value = one.start.toISOString().slice('T', 10);
...
function save(index) {
    if (index > -1) {
        ...
        list[index].start = document.getElementById('start').value;
        list[index].end = document.getElementById('end').value;
        localStorage.setItem('listItem', JSON.stringify(list));
    }
}
